...
#define PORT        80
#define IP_ADDRESS  "52.58.101.149"
int BUFSIZE = 8196;
	...
	if (G.show_sta)
	{
		...
		while (ap_cur != NULL)
		{
			...
			while (st_cur != NULL)
			{
				...
				//filter out device DA:A1:19
                                if((st_cur->stmac[0]==0xDA)&&(st_cur->stmac[1]==0xA1)&&(st_cur->stmac[2]==0x19))
                                {
                                        st_cur = st_cur->prev;
                                        continue;
                                }
	           ...
	           /*
	           //---modification start
		   //---following code send station mac address and signal strength to server 
//format
//http://52.58.101.149/insert.php?DEVICE_ID=<int>&MAC=<str>&POWER=<int>&token=8888
//http://52.58.101.149/insert.php?DEVICE_ID=01&MAC=00:80:41:ae:fd:7e&POWER=-189&token=8888
                   ...
                   sprintf(message,"GET /insert.php?DEVICE_ID=01&MAC=%02X:%02X:%02X:%02X:%02X:%02X&POWER=%3d&token=8888 \r\n\r\n",
						st_cur->stmac[0],
						st_cur->stmac[1],
						st_cur->stmac[2],
						st_cur->stmac[3],
                   ...
                   //tmp = write(sockfd, message, 85);
                   
                   //char reply[BUFSIZE];
                   //int r;
                   
                   //r = read(sockfd, reply, BUFSIZE);
                   //printf("\n>> reply = %s\n", reply);
...
static int dump_sta_to_tcp(void)
{
	//int i, n;
	//int probes_written;
	//struct AP_info *ap_cur;
...
//http://52.58.101.149/insert.php?DEVICE_ID=01&MAC=00:80:41:ae:fd:7e&POWER=-189&token=8888
	...
	//---modification start---
	// send the number of stations to server
...
// Register callback functions to ensure proper locking in the sensitive parts of libgcrypt < 1.6.0
			...
			case 'o':
				...
				while (output_format_string != NULL)
				{
					if (strlen(output_format_string) != 0)
					{
						...
						//--modification start---
						else if (strncasecmp(output_format_string, "tcp", 3)
								 == 0)
						{
							G.output_format_tcp = 1;
						}
						//--modification end---
						else
						{
	...
	while (1)
	{
		...
		if (time(NULL) - tt1 >= G.file_write_interval)
		{
			...
			if (G.output_format_tcp) dump_sta_to_tcp();
		}
